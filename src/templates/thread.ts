...
export function render(content: string, metadata: ContentMetadata): string {
  ...
  if (segments.length <= 1) {
    segments = content.split(/\n(?=(?:(?:Tweet|Post)\s+\d+[:.]\s*|\d+\s*[-:./)]\s))/i).filter(s => s.trim());
  }
  ...
  segments = segments.map(s => s.trim().replace(/^(?:(?:Tweet|Post)\s+\d+[:.]\s*|\d+\s*[-:./)]\s*)/i, ''));
  ...
  <div class="sv-block" data-block-id="${blockId}" ${copyDataAttributes(seg.trim())} style="background:var(--sv-surface);border:1px solid var(--sv-border);border-radius:12px;padding:1.25rem 1.5rem;box-shadow:var(--sv-card-shadow)">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem">
      <span style="color:var(--sv-muted);font-size:0.75rem;font-weight:600">${i + 1}/${total}</span>
    </div>
    <div style="font-size:0.95rem;line-height:1.6">
      <p ${editableDataAttributes(seg, { format: 'markdown', kind: 'paragraph' })} style="margin:0">${renderMarkdown(seg).replace(/\n/g, '<br>')}</p>
    </div>
    ${imageHtml}
  ...
  return `<div class="sv-thread" ${copyDataAttributes(content.trim(), { primary: true })} style="max-width:550px;display:flex;flex-direction:column;gap:0">
  ${cards}
</div>`;
}
